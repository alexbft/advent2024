...
pub fn solve(input: &str) -> usize {
    let grid: Vec<Vec<u32>> = input
        ...
        .collect();
    let mut scores: HashMap<Pos, HashSet<Pos>> = HashMap::new();
...
fn calc_score(
    grid: &Vec<Vec<u32>>,
    scores: &mut HashMap<Pos, HashSet<Pos>>,
    x: i32,
    y: i32,
) -> usize {
    ensure_reach(grid, scores, x, y);
    scores.get(&(x, y)).unwrap().len()
}

fn ensure_reach(grid: &Vec<Vec<u32>>, scores: &mut HashMap<Pos, HashSet<Pos>>, x: i32, y: i32) {
    if scores.contains_key(&(x, y)) {
        return;
    }
    let cell_value_opt = get_at(grid, x, y);
    if cell_value_opt == None {
        scores.insert((x, y), HashSet::new());
        return;
    }
    let cell_value = cell_value_opt.unwrap();
    if cell_value == 9 {
        scores.insert((x, y), HashSet::from([(x, y)]));
        return;
    }
    ...
    for dir in [(1, 0), (0, 1), (-1, 0), (0, -1)].iter() {
        ...
        if get_at(grid, nx, ny) == Some(cell_value + 1) {
            ensure_reach(grid, scores, nx, ny);
            score.extend(scores.get(&(nx, ny)).unwrap());
        }
    }
